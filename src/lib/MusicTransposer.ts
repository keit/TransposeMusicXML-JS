...
export class MusicTransposer {
  ...
  transposeNote(step: string, alter: number, octave: number, semitones: number): TransposedNote {
  ...
  transposeNoteElement(noteElement: NoteElement, semitones: number): NoteElement {
    ...
    const alter = pitch.alter ? (Array.isArray(pitch.alter) ? parseInt(pitch.alter[0]) : parseInt(pitch.alter as string)) : 0;
    const octave = pitch.octave ? (Array.isArray(pitch.octave) ? parseInt(pitch.octave[0]) : parseInt(pitch.octave as string)) : 4;
  ...
  transposeHarmonyElement(harmonyElement: HarmonyElement, semitones: number): HarmonyElement {
    ...
    if (root.step || root['root-step']) {
      ...
      const alterValue = alter ? (Array.isArray(alter) ? parseInt(alter[0]) : parseInt(alter as string)) : 0;
    ...
    if (harmonyElement.bass) {
      ...
      if (bass.step || bass['bass-step']) {
        ...
        const bassStepValue = Array.isArray(bassStep) ? bassStep[0] : bassStep as string;
        const bassAlter = bass.alter || bass['bass-alter'];
        const bassAlterValue = bassAlter ? (Array.isArray(bassAlter) ? parseInt(bassAlter[0]) : parseInt(bassAlter as string)) : 0;
  ...
  transposeMusicXML(musicXMLObject: any, intervalString: string): any {
    const semitones = this.parseInterval(intervalString);
    
    const traverseAndTranspose = (obj: any, parentKey: string | null = null): any => {
