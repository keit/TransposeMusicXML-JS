...
import { MusicXMLPlaybackParser, type PlayableNote } from '../lib/MusicXMLPlaybackParser';
...
export class MusicPlaybackService {
  ...
  async play(options: PlaybackOptions = {}): Promise<void> {
    ...
    let processedSequence = this.applyPlaybackOptions(this.currentSequence, options);
      ...
      const timeoutId = setTimeout(() => {
        if (this.isPlaying && this.pianoInstance) {
          try {
            ...
            this.pianoInstance.triggerAttackRelease(note.pitch, note.duration, Tone.now());
          } catch (error) {
    ...
    const totalDuration = Math.max(...processedSequence.map(n => n.startTime + n.duration));
    setTimeout(() => {
      if (this.isPlaying) {
        this.stop();
      }
  ...
  private applyPlaybackOptions(sequence: PlayableNote[], options: PlaybackOptions): PlayableNote[] {
    let processedSequence = [...sequence];
    
    // Apply transposing instrument transposition
    if (options.transposingInstrument && options.transposingInstrument !== 'concert') {
      processedSequence = this.transposeSequence(processedSequence, options.transposingInstrument);
    }
    ...
    // Apply tempo scaling (this is handled by Tone.Transport.bpm)
    
    return processedSequence;
  }
  
  private transposeSequence(sequence: PlayableNote[], instrument: 'bb' | 'eb'): PlayableNote[] {
    // Bb instruments sound a major second lower, so we transpose up a major second (+2 semitones)
    // Eb instruments sound a major sixth lower, so we transpose up a major sixth (+9 semitones)
    const semitones = instrument === 'bb' ? 2 : 9;
  ...
  private applySwing(sequence: PlayableNote[]): PlayableNote[] {
    ...
    return sequence.map((note, index) => {
      ...
      if (index > 0) {
        ...
        if (timeDiff > 0 && timeDiff < 0.5) { // Less than half a second apart
  ...
  getTotalDuration(): number {
    ...
    return Math.max(...this.currentSequence.map(n => n.startTime + n.duration));
  }
}